...
**Edge cases integrated into test cases:** Rather than scattering failure modes across notes, all edge cases and failure modes are captured in the test cases table within the Low-Level Tasks section, making them both reviewable and directly actionable for verification.
...
| Explicit error codes per failure type (400/422/404) | `.claude/rules.md` → FinTech Defaults; `card-data-lookup-specification.md` → Test Cases |
